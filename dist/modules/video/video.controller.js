...
export const createVideo = async (req, res) => {
    try {
        const { title, description, hashtags, links, channelId, category, language, visibility, } = req.body;
    ...
    try {
        ...
        return res.status(200).json({
            ...
            data: {
                video: {
                    ...videoData,
                    streamingUrl: video.transcodeStatus === "COMPLETE"
                        ? video.videoUrl
                        : null,
                },
            },
...
export const checkTranscodingStatus = async (req, res) => {
    try {
        const { videoId } = req.params;
        if (!mongoose.Types.ObjectId.isValid(videoId)) {
            return res.status(400).json({
        ...
        video.transcodeStatus = jobStatus.status;
        await video.save();
        return res.status(200).json({
            status: "success",
            data: {
...
export const incrementViewCount = async (req, res) => {
    try {
        ...
        const video = await Video.findByIdAndUpdate(videoId, { $inc: { totalViews: 1 } }, { new: true });
        if (!video) {
            return res.status(404).json({
                status: "fail",
                message: "Video not found",
