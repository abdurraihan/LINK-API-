...
export const createShort = async (req: Request, res: Response) => {
  try {
    ...
    const videoFile = req.file as Express.MulterS3.File;
...
export const checkShortTranscodingStatus = async (req: Request, res: Response) => {
  try {
    const { shortId } = req.params;
    ...
    short.transcodeStatus = jobStatus.status as any;
    await short.save();
...
export const incrementShortViewCount = async (req: Request, res: Response) => {
  try {
    const { shortId } = req.params;
...
export const getShortsByChannel = async (req: Request, res: Response) => {
  try {
...
export const updateShort = async (req: Request, res: Response) => {
  try {
    ...
    if (category) short.category = category;
    if (visibility) short.visibility = visibility;
...
export const publishShort = async (req: Request, res: Response) => {
  try {
    const { shortId } = req.params;
    ...
    if (duration) short.duration = duration;
